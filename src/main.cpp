...
const string strMessageMagic = "Elektron Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight+1 == 1)
    {
        ...
        else if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 149)
    {
		nSubsidy = 0 * COIN;
    }   	
		else if(pindexBest->nHeight+1 >= 150 && pindexBest->nHeight+1 <= 1560)
    {
        nSubsidy = 150 * COIN;   // 70 500
    }
		else if(pindexBest->nHeight+1 >= 1561 && pindexBest->nHeight+1 <= 5881)
    {
        nSubsidy = 30 * COIN;  // 129 600
    }
		else if(pindexBest->nHeight+1 >= 5882 && pindexBest->nHeight+1 <= 21880)
    {
        nSubsidy = 50 * COIN;  // 799 900
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Elektron", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Elektron - 2015 Cryptocurrencies spread continue!";
        CTransaction txNew;
        txNew.nTime = 1428152066;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1428152066;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2405688;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x595118b5e13401055624122ac5baf02b37964ba744d07d596ffb215f6bb3ef62"));
        block.print();
