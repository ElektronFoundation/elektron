/* Author: Yash yash@speedovation.com Owner: Elektron
 *
...
void HashMarketApi::ccex()
{
 ...
 //   QUrl url( QString("https://c-cex.com/t/btc-%1.json").arg(market) );
	QUrl url( QString("https://c-cex.com/t/ekn-btc.json").arg(market) );
    QNetworkReply* reply = nam->get(QNetworkRequest(url));
...
/*
CryptoTradeEx
https://api.cryptotrade.exchange/v1/market/stats/EKN/BTC
we need to pull info from “lasttradeprice”
market id will change to our market after launch.   Market 127 can be used for now.
*/
void HashMarketApi::CryptoTradeEx()
{
    QUrl url( QString("https://api.cryptotrade.exchange/v1/market/stats/EKN/BTC") );
    QNetworkReply* reply = nam->get(QNetworkRequest(url));

    QObject::connect(reply, SIGNAL(finished()),
                     this, SLOT(CryptoTradeExFinished()));
}
void HashMarketApi::CryptoTradeExFinished()
{
    QNetworkReply *reply = qobject_cast<QNetworkReply*> ( sender() );
    QJsonObject jsonObject = result(reply);
...
//    market = "AUR";
    ...
    emit updateMarket("CryptoTradeEx", QString::number(r,'f',8 ) );
